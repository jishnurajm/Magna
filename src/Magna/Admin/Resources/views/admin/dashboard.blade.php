<x-filament-panels::page>
    ...
    {{-- Owner-broadcast banners (Marketplace plugin's Announcements screen) —
         pinned above the widget grid, not part of the draggable masonry. --}}
    <livewire:magna-dashboard-notices />
    ...
    <style>
        ...
        /* Widgets declaring columnSpan = 'full' break out of the masonry
           columns entirely for a true full-width row, via the standard CSS
           multi-column "spanning element" feature — everything before it
           keeps filling columns up to this point, then columns resume fresh
           underneath it. Still one flat container, so drag reordering
           (Sortable) doesn't need to know the difference. */
        #magna-dashboard-widgets > [data-widget-key].magna-dashboard-widget--full {
            column-span: all;
    ...
    >
        @foreach ($this->getVisibleWidgets() as $widgetClass)
            <div
                ...
                class="relative group {{ $this->widgetIsFullWidth($widgetClass) ? 'magna-dashboard-widget--full' : '' }}"
            >
